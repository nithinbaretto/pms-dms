...
import type { SignatoryDetails, SignatoryMode } from "../types";

type SignatoryDetailsSectionProps = {
...
const RoleBadge = ({ source }: { source: SignatoryDetails["source"] }): ReactElement | null => {
  if (source === "karta") {
    return (
      <span className="rounded-full bg-[#FBF3E0] px-2 py-0.5 font-['Mulish',sans-serif] text-[11px] font-medium leading-none text-[#C08B2E]">
        Karta
      </span>
    );
  }
  ...
  if (source === "coparcener") {
    return (
      <span className="rounded-full bg-[#E8F1FF] px-2 py-0.5 font-['Mulish',sans-serif] text-[11px] font-medium leading-none text-[#2F6FED]">
        Co-Parcener
      </span>
    );
  }
...
const SignatoryDetailsSection = ({
  ...
  onRemove,
  onView,
}: SignatoryDetailsSectionProps): ReactElement => {
  const copy = HUF_ENTITY_DETAILS_COPY.signatory;
  const countOptions = signatories.map((_, index) => index + 1);
      ...
      {signatories.length > 0 ? (
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="inline-flex rounded-full bg-[#F0F2F5] p-0.5">
              {(["jointly", "any"] as const).map((option) => (
                <button
                  className={`h-8 rounded-full px-4 font-['Mulish',sans-serif] text-[13px] font-medium leading-none ${
                    mode === option
                      ? "bg-white text-[#93161E] shadow-sm"
                      : "text-[#5A6B7D]"
                  }`}
                  key={option}
                  onClick={() => {
            ...
            <p className="font-['Mulish',sans-serif] text-[12px] font-normal leading-[18px] text-[#71859B]">
              All the authorized signatories will be automatically selected
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {signatories.map((signatory) => (
              <div
                className="w-full max-w-[360px] rounded-[8px] border border-[#93161E] bg-white"
                key={signatory.id}
              >
                <div className="space-y-2 p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex min-w-0 flex-1 items-start gap-2">
                      <span className="mt-0.5 flex size-4 shrink-0 items-center justify-center rounded-[4px] bg-[#93161E]">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex items-center justify-between gap-2">
                          <p className="font-['Mulish',sans-serif] text-[12px] font-normal leading-none text-[#5A6B7D]">
                            {signatory.pan}
                          </p>
                          <RoleBadge source={signatory.source} />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-2 border-t border-[#EEEEEE] px-3 py-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <p className="font-['Mulish',sans-serif] text-[12px] font-normal text-[#231F20]">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <p className="break-all text-right font-['Mulish',sans-serif] text-[12px] font-normal text-[#231F20]">
                      {signatory.email}
                    </p>
                  </div>
                </div>
          ...
          {mode === "any" ? (
            <div className="max-w-full space-y-1">
              <p className="font-['Mulish',sans-serif] text-[12px] font-normal leading-none text-[#231F20]">
                Number of Signatory <span className="text-[#E8402F]">*</span>
              </p>
              <div className="relative">
                <select
                  className="h-9 w-full appearance-none rounded-[8px] border border-[#eeeeee] bg-white px-[14px] font-['Mulish',sans-serif] text-[14px] font-normal text-[#231F20] outline-none focus-visible:border-[var(--color-onboarding-primary)] focus-visible:ring-2 focus-visible:ring-[rgba(147,22,30,0.2)]"
                  onChange={(event) => {
